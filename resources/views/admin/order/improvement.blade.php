                                ...
                                <div class="btn-group nav">
                                    ...
                                    <a href="{{ route('not_started', ['project_id' => request()->project_id]) }}"
                                        ...
                                        <span style="font-size: 20px"
                                            class="badge badge-danger badge-pill ml-2">{{ \App\Models\Task::where('user_id', auth()->id())->where('status', 0)->where('project_id', request()->project_id)->count() ?? '0' }}</span>
                                    </a>
                                    <a href="{{ route('in_progress', ['project_id' => request()->project_id]) }}"
                                        ...
                                        Progress
                                        <span style="font-size: 20px"
                                            class="badge badge-dark badge-pill ml-2">{{ \App\Models\Task::where('user_id', auth()->id())->where('status', 1)->where('project_id', request()->project_id)->count() ?? '0' }}</span>
                                    </a>
                                    ...
                                    <a href="{{ route('completed', ['project_id' => request()->project_id]) }}"
                                        ...
                                        <span style="font-size: 20px"
                                            class="badge badge-dark badge-pill ml-2">{{ \App\Models\Task::where('user_id', auth()->id())->where('status', 5)->where('project_id', request()->project_id)->count() ?? '0' }}</span></a>
                                    <a href="{{ route('rejected', ['project_id' => request()->project_id]) }}"
                                        ...
                                        <span style="font-size: 20px"
                                            class="badge badge-dark badge-pill ml-2">{{ \App\Models\Task::where('user_id', auth()->id())->where('status', 6)->where('project_id', request()->project_id)->count() ?? '0' }}</span></a>
                                                            ...
                                                            <td class="text-center">
                                                                <div class="btn-group">
                                                                    @if (auth()->user()->roles[0]['name'] == 'superadmin')
                                                                        <a href="{{ route('create_post', ['project_id' => request()->project_id, 'task_id' => $task->id]) }}"
                                                                            class="btn  btn-warning mx-2"><i
                                                                                class="fa fa-solid fa-spinner fa-spin-pulser mr-2"></i>
                                                                            {{ $task->status == 1 ? 'Open The Post' : 'Wiating Approval from Client' }}
                                                                        </a>
                                                                    @else
                                                                        <a href="{{ route('show_client_post', ['project_id' => request()->project_id, 'task_id' => $task->id]) }}"
